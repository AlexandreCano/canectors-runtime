...
- **`dataField` des filtres d'appel ne réduit plus rien** (25.3) : un tableau non vide reste un tableau et exige `mergeStrategy: append` (`merge`/`replace` remontent une erreur), un tableau vide est un résultat fonctionnel (`[]` en `append`, record inchangé en `merge`/`replace`), un scalaire ou un champ absent est une erreur de validation qui traverse `onError` sans être retentée. `soaputil.ValueAsRecordMap` (enrobage implicite `items`/`value`) a été supprimée. Le chemin est résolu par `recordpath`, donc `dataField: data.results` fonctionne des deux côtés.
...
- **`schedule` est seulement contraint par `minLength = 9` dans le schema** : un CRON 5 champs (`* * * * *`) satisfait donc cette validation de longueur. Ne pas documenter une exigence 6 champs sauf si elle est effectivement imposée ailleurs.
